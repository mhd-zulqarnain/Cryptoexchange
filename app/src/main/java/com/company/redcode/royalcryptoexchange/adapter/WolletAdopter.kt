...
class WolletAdopter (var datalist: ArrayList<Wallet>):
        RecyclerView.Adapter<WolletAdopter.ViewHolder>() {

        var  ctx:Context? = null
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.wollet_cardview,parent,false)
        ctx = parent.context
        return ViewHolder(v);
    }
    ...
    private fun showSendDialog() {
        val view: View = LayoutInflater.from(ctx!!).inflate(R.layout.dialogue_wollet_send, null)
        val alertBox = android.support.v7.app.AlertDialog.Builder(ctx!!)
